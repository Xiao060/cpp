...
void process(WFHttpTask* httpTask) {
    ...
    // 3. 解析请求
    // GET /login?username=xxxx&password=yyyy
    // 获取 用户名 及 密码
    string usernamePair = uri.substr(0, uri.find("&"));
    string passwordPair = uri.substr(uri.find("&")+1);
