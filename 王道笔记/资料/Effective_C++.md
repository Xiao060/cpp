...
4. 赋值运算符函数
    ...
    1. 若 数据成员 为 <mark>reference / const</mark>, 则不会生成 默认的 赋值运算符函数
...
1. 拷贝/赋值 函数 <mark>delete</mark>

2. 将 拷贝 / 赋值 函数 声明为 <mark>私有</mark>, 并且不需要实现;  
    若使用 会导致 连接错误;

3. 创建一个 不含数据成员的基类, 将其 拷贝 / 赋值 函数设为 <mark>私有</mark>, 并用自己的类 继承它;  
    若调用, 会在编译期出错;  
    but 可能导致多重继承
...
5. 若 class <mark>含有任何 virtual 函数, 析构函数 也应设为 virtual</mark>
...
1. 调用 派生类构造函数 函数时 会先调用 基类构造函数, 因为此时 派生类还未构造完成, 所以此时的 virtual 还不是 虚函数, 或者说 此时的 **对象类型是 基类**, 所以执行的 virtual 函数 其实是 基类的 函数

2. 执行 析构函数时 也是一样, 先调用 派生类析构函数, 然后调用成员子对象析构函数, 最后调用 基类子对象析构函数; 析构函数 执行时 会先确定它的 对象类型, 若在 基类析构函数中 执行 virtual 函数, 则执行的是 基类的 虚函数
...
1. 加入 **等同测试** `this != &rhs`, 保证 自我复制的安全性;  
    but new 可能导致 异常安全性 问题

2. <mark>copy and swap</mark>  
    使用 拷贝构造 创建一个 参数的副本, 将 this 对象 与 副本对象 进行交换

    ![](https://xiao060.oss-cn-hangzhou.aliyuncs.com/md/202310012328943.png)

### 12. 赋值对象时应确保复制 "对象内的所有成员变量" 及 "所有 base class 成分"

1. 派生类对象 构造函数中 若没有 显式调用 基类构造, 则会 **隐式调用** 基类的默认构造函数(不推荐);  
...
5. `shared_ptr` 是 <mark>共享所有权</mark> 的智能指针, 支持值语义(拷贝 / 赋值) 与 移动语义;  
    ...
    but 循环引用会出现 <mark>内存泄漏</mark>, 解决方案 `weak_ptr`

6. 智能指针 析构函数 默认执行 `delete ...` 操作;  

    若 资源储存在数组中, 建议使用 vector 等 STL, STL 中储存 智能指针;  

    若 资源释放方式 不为 delete, 例 `f.close()`, 需要自定义 <mark>删除器</mark>(auto_ptr 不存在);  
    `unique_ptr` 删除器 位于模板参数中  
    `shared_ptr` 删除器 位于构造的参数中
...
2. RAII 常采用的 复制行为:
    ...
    1. 禁止拷贝 (设为私有 / 继承 uncopyable 类, 见 [06](#06)) (unique_ptr)
...
1. 往往 需要在 资源管理类(智能指针)外 访问原始资源
    ...
    2. 隐式转换, 例 自定义类型转换函数, `xxxx operator xxxx() const { ...; }` () FIXME:
