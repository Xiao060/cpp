...
2. `for_each`
    ...
    3. `bind1st` / `bind2nd` / `bind` [详细介绍](#bind)

3. `remove_if`

4. `bind1st` / `bind2nd` / `bind` <div id="bind" ></div>
    ...
    3. `bind`
        ...
        2. 可以绑定 <mark>普通函数 / 成员函数 / 函数对象</mark> [ChatGpt](https://poe.com/s/ssClgFym2aDVgMgeJ9HV)
        ...
        // 第 2/3 使用 占位符, 函数调用时传递, 2 对应 第 4 个 实参, 3 对应第 3 个 实参; 多余的 实参 会被丢弃
        int number = 100;
        using namespace std::placeholders;
